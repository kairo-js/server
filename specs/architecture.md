...
propertiesから`BP/manifest.json`が生成されることを画面上で明示します。完成時には、ソース、初期BP成果物、manifest、UUID、任意のpack icon、README、Git設定、パッケージ設定を含むプロジェクトZIPをブラウザ内で生成します。入力ファイルと生成物はサーバーへ送信しません。
