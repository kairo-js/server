...
| `addon_id_intents` | Builderで生成されたIDの90日間の利用意向（匿名識別子はハッシュ化） |
| `project_generation_events` | 端末・言語・実行環境・GitHub・パッケージマネージャー・品質ツールの匿名生成統計 |
| `organizations` | Organizationのslug、表示名、公式・認証状態 |
| `organization_members` | Organizationとユーザーの所属、owner・admin・member権限 |
| `addon_owners` | アドオンを所有する個人またはOrganization |
...
`STORAGE_DRIVER=filesystem`では`STORAGE_PATH`配下へ保存し、`STORAGE_DRIVER=r2`ではCloudflare R2のS3互換APIへ保存します。投稿時にZIP形式、危険な相対パス、展開後容量を検証し、DBにはSemVer、サイズ、SHA-256、投稿者を記録します。R2の認証情報や内部オブジェクトキーは公開APIへ返しません。
...
POST   /api/v1/addons
POST   /api/v1/addons/{id}/versions
POST   /api/v1/tokens
DELETE /api/v1/tokens/{id}
GET    /api/v1/addon-ids/{id}/availability
POST   /api/v1/development-projects/generated
...
アドオンの所有者と、各バージョンを実際に投稿したユーザーは分離します。個人またはOrganizationがアドオンを所有し、バージョンには監査用の投稿ユーザーを記録します。

- `kairo-js` Organizationはmigrationで公式・認証済みとして作成する
- GitHub OAuthで確認したloginが`shizuku86`の場合だけ、`kairo-js`のowner権限を冪等に付与する
- `kairo`と`kairo-database`は`kairo-js`が所有する
- Organization権限は`owner`、`admin`、`member`の3段階とする
- `official`と`verified`は一般ユーザーが変更できないシステム管理属性とする
- Organization作成だけでは公式・認証済みにならない
- アドオン作成時は個人所有または所属Organization所有を指定する
- Organizationの`owner`と`admin`だけが、そのOrganization所有のアドオンを作成できる
- 予約IDは通常の個人・Organizationでは作成できず、`kairo-js`だけが公式IDを登録できる

Webの`/{locale}/publish`はログイン必須のアドオン登録画面とします。ログインユーザー本人と、owner/admin権限を持つOrganizationだけを所有者候補として表示します。ID・表示名・任意説明を登録し、成功後は最初のバージョン投稿へ進む導線を表示します。member権限のOrganizationは新規アドオンの所有者候補に表示しません。
...
新しいバージョンの投稿では、ZIPと同じmultipart requestに`manifest`フィールドを含めます。`addonId`と`version`は投稿先およびフォーム値と一致する必要があります。通常のアドオンでは`kairo`と`kairo-database`が必須依存関係です。依存関係のルートとなる`kairo`は両方を必須とせず、`kairo-database`は`kairo`だけを必須とします。すべてのアドオンで自己依存を禁止します。
...
manifestはバージョン情報APIの`manifest`として返し、launcherがMinecraft互換性の判定と依存アドオンの解決に利用します。既存データ移行のため、manifest導入前に投稿されたバージョンではこのフィールドが存在しない場合があります。
...
- `kairo` と `kairo-database` は編集できない必須依存関係として `^1.0.0` を自動的に追加する
- Kairo系の追加依存関係はIDとバージョン条件の行を任意個追加できるようにする
- `official` と `approved` はユーザーが選択できず、サービス側で後から付与する
- 実験版モジュールを選択した場合、またはMinecraft依存バージョンに `alpha` / `beta` が含まれる場合は `experimental`、それ以外は `stable` を自動付与する
...
- 実験版: `@minecraft/server-admin`、`@minecraft/server-gametest`、`@minecraft/server-net`、`@minecraft/server-editor`、`@minecraft/debug-utilities`、`@minecraft/diagnostics`、`@minecraft/server-graphics`
- 実験版は初期状態で折りたたみ、必要な場合だけ展開する
...
実験版モジュールを選択した場合、または選択中のMinecraft依存バージョンに`alpha`または`beta`が含まれる場合、propertiesのシステムタグは`experimental`とします。

現在の `SupportedTagType` はシステムタグだけを列挙しているため、カスタムタグを正式対応する際は、システム管理タグとユーザー定義タグを区別できる型へ変更します。

### プロジェクト作成ウィザード（初期版）

開発サポートの入口は3段階のウィザードとします。

1ページ目では次を選択します。

- 開発端末: Windows（推奨）、スマートフォン、macOS / Linux
