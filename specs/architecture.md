...
Minecraft Script APIモジュールは用途と成熟度でグループ化します。

- 主要: `@minecraft/server`、`@minecraft/server-ui`
- 上級者向け: `@minecraft/server-admin`、`@minecraft/server-gametest`
- BDS向け: `@minecraft/server-net`
- 開発段階: `@minecraft/server-editor`、`@minecraft/debug-utilities`、`@minecraft/diagnostics`、`@minecraft/server-graphics`
- `@minecraft/server-editor-private-bindings` はmanifestへ記載するものではないため候補に含めない

既知バージョン候補は次のとおりです。

- `@minecraft/server`: `2.8.0`から`2.0.0`までの各minor。Minecraftは1.xも許可するがKairoは2.0.0以上のみ対応
- `@minecraft/server-ui`: `2.1.0`、`2.0.0`。Kairoは2.0.0以上のみ対応
